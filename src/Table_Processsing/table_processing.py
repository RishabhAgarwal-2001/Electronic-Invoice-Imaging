...
target=open("IB1.txt","w")
file=open("F:\Vacation_TP\D2C_Flipkart\Electronic Invoicing using Image Processing\output_edit.html","r")
...
for itr in range (0,len(text[0])-1):
    ...
    while int(boundaries[itr+1])-int(r_bounds[itr])<7:
        ...
        for thisitr in range (1,len(text)):
            try:
    ...
    for itr in range(len(text[j])):
        if cell>=len(boundaries)-1:
            try:
                ...
                sheet1.write(j,cellmax,text[j][itr][1:])
            continue
        while cell<len(boundaries)-1 and int(d2[j][itr])>int(boundaries[cell+1]):
            cell=cell+1
        att=cell
        while True:
            try:
